...
def process_queries(queries, db_path):
    ...
    for old_id, query, folder_name in tqdm(queries, desc="Processing queries"):
        ...
        for stored_query in stored_queries:
            if fuzz.ratio(query, unidecode(stored_query)) >= 95:
                cursor.execute("SELECT old_ids FROM queries WHERE query = ?", (stored_query,))
                existing_old_ids = cursor.fetchone()[0]
                new_old_ids = existing_old_ids + ',' + old_id_with_folder
                cursor.execute("UPDATE queries SET old_ids = ? WHERE query = ?", (new_old_ids, stored_query))
                stored_queries[stored_query].append(old_id_with_folder)
                found_similar = True
                break

        if not found_similar:
            stored_queries[query] = [old_id_with_folder]
            try:
                cursor.execute("INSERT INTO queries (query, old_ids) VALUES (?, ?)", (query, old_id_with_folder))
